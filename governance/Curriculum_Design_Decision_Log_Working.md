...
### PD-067 — Teacher Answer Key may be assembled before the Assignment exists
**Date:** 15.08.26 · **Class:** project-wide · **Authority:** Principal
**Decision.** Run Book **§6.11** requires the AK to be generated *"once **every** worksheet, the Performance Test **and the Assignment** are final"*, and **§3.15** places `Assignment` last in the AK's fixed order. That ordering is **released**: an AK may be assembled once every **worksheet and the PT** are final. The Assignment section is added to the AK when the Assignment is written; until then the AK omits it and says so.
**What led to this.** The requirement had already been overtaken by practice twice without a ruling. **`extracts/C3/C3B07-AK.md`** and **`extracts/C4/C4B06_AK.md`** — the only two AKs in the repository — both carry CW1–HW4, the PT and the dictation list, and **neither carries an Assignment section**. Raised at Phase 1 of the `C3B08-AK` build, where C3B08 likewise has no Assignment.
**Reasoning.** §6.11's stated rationale is staleness — *"a key written next to a worksheet still under revision goes stale silently."* That risk attaches to the **worksheets and the PT**, which the AK keys item by item. The Assignment is a separate artefact on a separate cycle; holding the whole key hostage to it leaves a teacher marking a completed week's work with no key at all, which is the concrete harm §3.15 exists to prevent.
**Scope limits.** (1) The worksheet-and-PT precondition is **unchanged** — an AK is still never written alongside a sheet under revision. (2) §6.11's programmatic consistency check is **unchanged and still binding** on every artefact the AK does cover. (3) The AK remains derived, unversioned and regenerate-only (§3.12 r.1); adding the Assignment section later is a regeneration, not a version. (4) An AK shipped without the Assignment section **states that on its face**, so a teacher does not read a missing section as a missing artefact.
**Affected files:** Run Book §3.15 and §6.11 — ⚑ **edit pending**, to be applied at the Run Book's next reissue (forward-only, Charter §K.3). `extracts/C3/C3B07-AK.md` and `extracts/C4/C4B06_AK.md` are retrospectively compliant. **Status:** Ruled; Run Book alignment pending.

---

### PD-068 — Bare naming-word items exempt from PD-036; AK word lists duplicated in the master
**Date:** 15.08.26 · **Class:** project-wide · **Authority:** Principal
**Two rulings, taken together on the C3B08 AK build.**

**(a) PD-036 cross-sheet repetition — bare naming-word items are exempt.** A graded item whose entire text is a single naming word or name (the `CW1/HW1 Part B` shape, *"Maryam → ______"*) **may repeat across a paired sheet** without breaching the PD-036 zero threshold. The roster is fixed at eleven names (Charter §H.5) and the pool's concrete singulars are finite; across a CW/HW pair drawing four gender/number categories from that set, collision is arithmetic rather than authorial. **Applied immediately to CR-040** — `Maryam` stands on both `C3B08-CW1 #16` and `C3B08-HW1 #12`; no edit, and v1.5's he 2 · she 2 · it 2 · they 2 balance repair stands unaltered.
**Scope limit.** The exemption reaches **bare naming-word items only**. Any item carrying a sentence, a clause or a carrier phrase remains under the zero threshold — that is the CR-009 and v1.1 `Yusuf` case, which was a Part B *sentence* item, and it is not reopened. `run_all.py`'s `gate_cross_sheet_repetition()` should gain the exemption as a declared part-shape flag rather than a hand-waived FAIL; **⚑ check edit pending**, and until it lands the FAIL is expected on this block and is answered by this ruling.

**(b) PT dictation and self-try word lists print in both the master and the AK.** §3.15 r.3 places them in the AK; §3.12 r.2 keeps them in the master (teacher copy) and out of student extracts. Both stand — the lists are **duplicated**, not moved.
**Risk accepted and mitigated.** Two copies of one list is the CR-027 drift shape. Mitigation, binding on this and every later AK: the AK's copy carries a **"regenerated from the master — do not edit here"** line, and the master's copy carries a pointer to the AK. Any change to a dictation or self-try set is made in the master and the AK regenerated (§3.12 r.1), never the reverse.
**Affected files:** Run Book §3.15 r.3 and §6.5 cross-sheet line — ⚑ **edits pending** at next reissue. `audits/scripts/run_all.py` — exemption flag pending. **Status:** Ruled; applied to C3B08.
