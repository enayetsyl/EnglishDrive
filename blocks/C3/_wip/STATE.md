...
|---|---|---|
| Block tier | **First-teaching.** Every C3 grammar block is first-teaching of its rung | Drive Plan v1.9 §4 preamble |
...
| Drive Plan version | **v1.9** — reissued this session by **PD-052** (was v1.8 at orientation) | this session; Charter §K.3 forward-only |
| Charter / Run Book | **Charter v1.6** — reissued this session by **PD-053** (was v1.5 at orientation) · **Run Book v1.17** | this session; Charter §K.3 forward-only |
...
> **Personal pronouns** — subject (I, **you**, he, she, it, we, they) and object (me, **you**, him, her, us, them); they **replace a noun**.
...
**⚑ F2 — LIVE, mitigated by hand (Q7). The whole block's graded answers are function words — exactly the CR-032 PATTERN.**
...
**⚑ F3 — RESOLVED by PD-051/PD-052. The taught pronouns are not in File 2 and needed a declared status.** *I, he, she, it, we, they,
...
is for**. **Ruled: block-local, twelve words including *you*** (Q1, Q2 — see §3).

**⚑ F4 — RESOLVED by PD-053 (Charter v1.6). House-name roster: CLAUDE.md and Charter §H.5 disagreed.** Charter §H.5 lists male *Nusair, Yusuf,
...
one breach for another. **Ruled: Charter §H.5 amended to add both names; C3B07 not reopened** (Q5 — see §3).
...
Bore directly on **Q2**, which the Principal ruled on the plan's internal contradiction rather than on the book.

---

## §3 — Phase 2 rulings (ALL CLOSED — Principal, 10.08.26)

| Q | Question | Ruling | Instrument |
|---|---|---|---|
| **Q1** | Status of the eleven pronouns | **Block-local (PD-012/PD-035)** — as recommended. Counted toward W7 load, never dictation/spelling, not added to File 2, gradeable in-block only | **PD-051** |
| **Q2** | Is *you* taught? | **Taught AND graded** — the Principal went beyond the agent's cap-preserving "teach, don't grade" recommendation. §2 row 8 cap widened | **PD-052** · Drive Plan **v1.9** |
| **Q3** | Does *this/that* touch the sheets? | **Teacher script + Clue Card only.** No *this/that* on any student sheet, graded or carrier | STATE record; no PD needed (interpretation within the cap) |
| **Q4** | PT shape | **Standalone `C3B08-PT`** on the C3B07 house layout: dictation Part A (10 words) → grammar parts → 4-mark self-try. **Total to be set from the blueprint's part breakdown** | STATE record; PD-043 discharged |
| **Q5** | House-name roster | **Charter §H.5 amended** to add *Maryam* and *Fatima* — the Charter list was incomplete, not CLAUDE.md wrong. C3B07 **not** reopened; CR-029 becomes retroactively correct | **PD-053** · Charter **v1.6** |
| **Q6** | Roll size | **17 confirmed.** Closes the "roll = 17 unverified" item carried from C3B07. Four Exit Check sets × 17 prompts | STATE record |
| **Q7** | CR-032 PATTERN check | **Apply trigger discipline by hand**; the manifest-completeness check is **not** built before this block. Every item declares its antecedent noun as `trigger` from the manifest's first line | STATE record; CR-032 stays PATTERN, unbuilt |
| **Q8** | Block 7 delivery status | **Delivered — locked.** Corrections propagate forward only (Charter §K.3). C3B07 frozen as promoted | STATE record |

### What the rulings change about the build

- **Twelve** block-local words, not eleven: *I, you, he, she, it, we, they, me, him, her, us, them.*
- **Grading surface is 100% block-local.** No graded answer in this block traces to a File 2 word. The `trigger`
  field is therefore the **only** thing standing between this block and a vacuous held-word gate (Q7 + F2).
  Every item's trigger is its **antecedent noun**, which is also the word that makes the item answerable.
- **W7 load carries twelve counted words** on top of the 34-word batch, on a week Drive Plan §4 already flags as a
  compound peak. Documented in PD-051 so that if the §3 retention gate softens at W7, the cause is on record.
- **No *this/that* anywhere on a sheet** — so no dual-class AK marking note is needed (CR-015 avoided by design).
- **Eleven house names available** (Charter v1.6), which materially eases the antecedent variety this block needs:
  a pronoun block needs many distinct third-person referents and cannot lean on two or three names.

### ⚑ Still open after these rulings

1. **Cross-class replication of PD-052 — NOT executed.** The §2 boundary note says row 8 *"replicates to Classes
   1/2/4/5."* If *you* is missing there too, four plans carry the same defect — and **C1 Block 6 (Pronoun) is
   already promoted**, so it may be delivered with the gap. **Needs a separate ruling. No other plan touched.**
2. **CR-032 remains PATTERN with no check built** (Q7 ruled build-first-check-later). §5A's obligation is
   deferred, not discharged.
3. **File 2 batch-size declarations still wrong** (F1) — recorded, not corrected.
...
2. **Item shapes**, all mirroring Annual Q3's **no-word-box cloze over a connected passage**, plus the subject/object
   contrast the paper's blanks (c) and (e) require.
3. **The `trigger` contract** — the antecedent noun declared per item, and how the manifest represents a
   passage-cloze part so no graded placement is invisible to a gate (CR-028).
4. **De-patterning strategy for a small answer set.** With twelve possible answers heavily concentrated on
   *he/she/it/they*, CR-020's alternation trap is the live risk; the key must break runs **and** alternation, and
   each HW key must not be positionally derivable from its CW (CR-006).
5. **PT part structure and mark total** for `C3B08-PT`, for the Principal to set (Q4).
